...
- `config` isn't necessarily the sample's object exactly as exported: the client adds `options.maintainAspectRatio: false` when a sample's own config doesn't already set it, so the chart fills the container the fence's `height=` sizes (see [Fence meta parameters](#fence-meta-parameters)) instead of maintaining an aspect ratio derived from the canvas's own width/height attributes.
...
This is what the `height=500` in the example above sets — a 500px-tall chart instead of the 420px default. The canvas's *container* gets this height (as a CSS custom property) and the chart fills it, following Chart.js's own recommended pattern for a responsive chart in a sized container (`position: relative` on the container, the canvas positioned to fill it absolutely) — so the client also defaults `options.maintainAspectRatio` to `false` for every entry, unless a sample's own config already sets it, since responsive sizing and a fixed aspect ratio pull the canvas's size in opposite directions. Parsing of all three fence parameters is covered by `test/remark.test.js` (e.g. `height=600` and `height=500` fences, a default-height case, and dedicated cases for `title=`/`chartTitle`/`code=`); the container/canvas sizing itself is covered by the browser suite's pixel fixtures (`test/browser/fixtures.spec.js`).
...
Applying a selection updates the **existing** chart instance in place — `chart.data = ...`, `chart.options = ...`, `chart.update()` — rather than destroying it and starting over, so the chart transitions between states instead of replaying its entry animation on every click. The configuration handed to it is built fresh each time (a new top-level object, never a poke into the live one), which is what keeps the result independent of the order options happen to resolve in and makes a choice targeting `options` behave the same way as one targeting `data`. The one exception is a choice that would change a chart's `type`: Chart.js has no supported way to turn a live instance into a different controller/element/scale trio, so that throws instead of silently doing nothing.
